...
public:
    ...
    int romanToInt(string s) {
        int length = s.length();//获取罗马字符串长度
        char single = s[0];//截取第一个字符
        if(length==1){//如果字符串长度为一，直接取值返回
        ...
        if(stringMap.find(db)!=stringMap.end()) {
            sum += stringMap.at(db);//取值
            if(length==2)//如果输入字符串只有两个字符，直接返回
                return sum;
            romanToInt(s.substr(2));//将剩下的字符串迭代
        } else {//没有特殊字符串匹配，则只取第一个字符求值
            sum += charMap.at(single);
